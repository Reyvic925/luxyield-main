    ...
    <div className="max-w-xl mx-auto p-4">
      ...
      <form onSubmit={handleAmountSubmit} className="glassmorphic p-6 rounded-xl mb-8">
        ...
        <input
          ...
          className="w-full theme-aware-bg-primary border theme-aware-border theme-aware-text rounded-lg py-3 px-4 mb-2 focus:border-gold focus:outline-none"
        ...
        <div className="glassmorphic p-6 rounded-xl mb-8">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-8">
            {COINS.map(chain => (
              <button
                key={chain}
                className={`px-6 py-3 rounded-xl font-bold border-2 shadow-lg text-lg transition-all duration-200 ${selectedChain === chain ? 'bg-gold text-black border-gold scale-105' : 'theme-aware-bg-secondary theme-aware-border theme-aware-text-secondary hover:border-gold'}`}
              ...
              >
                ...
                <div className="text-xs theme-aware-text-secondary font-normal mt-1">{CRYPTO_DISPLAY[chain] || chain}</div>
              </button>
            ))}
          </div>
            ...
            <div className="mb-8">
              ...
              <div className="flex flex-wrap justify-center gap-3">
                {(NETWORKS[selectedChain] || ["Mainnet"]).map(net => (
                  <button
                    key={net}
                    className={`px-4 py-2 rounded-lg font-bold border transition-all duration-200 ${selectedNetwork === net ? 'bg-gold text-black border-gold scale-105' : 'theme-aware-bg-secondary text-gold border-gold/30 hover:border-gold'}`}
          ...
          {selectedChain && selectedNetwork && (() => {
            ...
            if (!wallet || !wallet.address) {
              return (
                <div className="flex flex-col items-center mt-6 p-6 rounded-2xl bg-gradient-to-br from-neutral-900 to-neutral-800 shadow-2xl border border-gold">
                  <div className="font-bold text-lg mb-2 uppercase tracking-widest text-gold">{selectedChain} <span className="text-xs theme-aware-text-secondary">({selectedNetwork})</span></div>
            ...
            return (
              <div className="flex flex-col items-center mt-6 p-6 rounded-2xl bg-gradient-to-br from-neutral-900 to-neutral-800 shadow-2xl border border-gold">
                <div className="font-bold text-lg mb-2 uppercase tracking-widest text-gold">{selectedChain} <span className="text-xs theme-aware-text-secondary">({selectedNetwork})</span></div>
                <QRCodeSVG value={wallet.address} size={160} className="mb-4 border-4 border-gold rounded-2xl bg-white dark:bg-neutral-900" />
                <div className="mb-2 break-all text-gold text-lg font-mono theme-aware-bg-secondary px-4 py-2 rounded-lg shadow-inner">{wallet.address}</div>
      ...
      <div className="glassmorphic p-4 rounded-xl overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {history.length === 0 && (
              <tr><td colSpan={3} className="text-center theme-aware-text-secondary py-6">No deposits yet</td></tr>
            )}
            {history.map(dep => (
              <tr key={dep._id}>
