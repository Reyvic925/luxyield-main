...
export default function CryptoNewsFeed() {
  ...
  return (
    <div className="glass-card p-6 rounded-xl mb-6 shadow-lg">
      <h3 className="font-bold text-lg mb-4 theme-aware-text">Live Crypto News</h3>
      {loading ? <div className="theme-aware-text-secondary">Loading...</div> : news.length === 0 ? <div className="theme-aware-text-secondary">No news found.</div> : (
        <ul className="space-y-3">
          {news.map(item => (
            <li key={item.id}>
              ...
              <div className="text-xs theme-aware-text-secondary">{item.published_at ? new Date(item.published_at).toLocaleString() : ''}</div>
            </li>
          ))}
        </ul>
      )}
